...
#
# ~88-90% of cuBLAS on 8192^3 fp16 (H100). At this block config, num_stages=4
# is already at the shared-memory ceiling (232KB on Hopper) -- num_stages=5+
# and larger block_m/block_n (e.g. 256x256) fail to fit, so deeper pipelining
# isn't available as a lever. block_k=128 (now buildable thanks to the
# TMA-segment-splitting support in cp_async_tensor.py) does not beat block_k=64.
