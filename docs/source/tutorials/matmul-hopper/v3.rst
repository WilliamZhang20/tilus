...
- Two :meth:`tma.global_to_shared() <tilus.lang.instructions.tma.TmaInstructionGroup.global_to_shared>`
  calls load A and B into ``sa[stage]`` / ``sb[stage]``. These are issued by the
  whole 32-thread warp: 32 threads is the granularity the TMA unit wants at the
  SASS level, and the hardware elects one lane to drive the descriptor.
...
Warp specialization lifts the kernel to **563 TFLOPS** (1.95 ms), 11% ahead of V2
and 4% ahead of V1. The autotuner chooses the *same* configuration as V2 --- 2
